...
- **Auth settings:**
  ...
  - **Phone / SMS sign-in is not used.** `auth.sms.enable_signup` is `false` in the cloud project, so nobody can sign up by phone, and Owners sign in with email and password. The Twilio provider itself is **still switched on** (`auth.sms.twilio.enabled = true` on the remote, `false` locally), along with `auth.sms.enable_confirmations`. The CLI cannot turn it off: `config pull` skips the key with `would_invalidate`, because it cannot represent `enabled = true` without an `account_sid`, and `config push` has no per-key flag. It has to be switched off by hand in *Authentication → Sign In / Providers → Phone* — the master provider toggle at the top of the section, not the phone-confirmations setting below it. Tracked as [#17](https://github.com/akashaf/virtual-queue-system/issues/17).
